...
def get_logger(args):
    logger_name = "{}_".format(args.tta_method) + datetime.now().strftime("%Y%m%d_%H%M%S") + ".txt"
    if not os.path.exists(args.output):
        os.makedirs(args.output, exist_ok=True)
    save_path = os.path.join(args.output, f"{args.dataset}_{args.corruption_modality}")
    ...
    logging.basicConfig(
        ...
        datefmt="%y/%m/%d %H:%M:%S",
        handlers=[
            logging.FileHandler(os.path.join(save_path, logger_name)),
            logging.StreamHandler()
        ])
    logger = logging.getLogger(__name__)
    logger.info("Start testing the model with TTA method: {}".format(args.tta_method))
    logger.info("GPU: {}".format(args.gpu))
    logger.info("dataset: {}".format(args.dataset))
    logger.info("corruption_modality: {}".format(args.corruption_modality))
    logger.info(args)
