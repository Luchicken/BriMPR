...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)  # 1/3, 768, (16, 16), (16, 16)
...
class CAVMAEFT(nn.Module):
    ...
    def forward(self, a, v, mode, test=False):
        # multi-modal fine-tuning, our default method for fine-tuning
        ##############################################
        if test:
            return self.forward_eval(a, v, mode)
        ##############################################
        ...
        # finetune with only image (and inference with only video when the model is finetuned with only image)
        elif mode == 'videoonly':
            v = self.patch_embed_v(v)
            v = v + self.pos_embed_v
            v = v + self.modality_v
